...
// Title:              Board.java
// Files:              Constants2048.java, Gui2048.java, PA5Tester.java
...
/**
 * The class for the game board object, its constructors, and its methods.
 *
 * Bugs: None known
 *
 * @author Rocky Hankin
    ...
    */
    ...
    public void printBoard() {
        ...
        for(int row = 0; row < this.grid.length; row++)
        {
            for(int column = 0; column < this.grid[row].length; column++)
            {
                System.out.printf("%5d", this.grid[row][column]);
            }
            System.out.println();
        }
    }
    ...
    private boolean canMoveRight() {
        for(int row = 0; row < this.grid.length; row++)
        {
            for (int column = 0; column < this.grid[row].length-1; column++)
            {
                if(this.grid[row][column] != 0)
                {
    ...
    private boolean canMoveLeft() {
        ...
        for(int row = 0; row < this.grid.length; row++)
        {
            for (int column = 1; column < this.grid[row].length; column++)
            {
     ...
     */
    private void moveUp() {
        for(int row = 0; row < this.grid.length - 1; row++) //sorting for loop
        {
            for(int column = this.grid[row].length - 1; column >= 0; column--)
            {
                if(this.grid[row][column] == 0) //finds a zero entry
                {
                    for(int i = row + 1; i < this.grid.length; i++) //finds a nonzero entry below it
                    {
                        if (this.grid[i][column] != 0)
                        {
                            this.grid[row][column] = this.grid[i][column]; //swaps them
        ...
        for(int row = 0; row < this.grid.length - 1; row++)
        {
            for(int column = this.grid[row].length - 1; column > 0; column--)
            {
                if(this.grid[row][column] == 0)
                {
                    for(int i = row + 1; i < this.grid.length; i++)
                    {
                        if (this.grid[i][column] != 0)
                        {
                            this.grid[row][column] = this.grid[i][column];
                            this.grid[i][column] = 0;
                            break;
                        }
                    }
    ...
    private void moveDown() {
        for(int row = this.grid.length - 1; row > 0; row--) //sorting for loop
        {
            for(int column = 0; column < this.grid[row].length; column++)
            {
                if(this.grid[row][column] == 0) //finds a zero entry
                {
                    for(int i = row - 1; i >= 0; i--) //finds a nonzero entry below it
                    {
                        if (this.grid[i][column] != 0)
                        {
                            this.printBoard();
    ...
    private void moveLeft() {
        for(int row = 0; row < this.grid.length; row++)
        {
            for(int column = 0; column < this.grid[row].length - 1; column++)
            {
                if(this.grid[row][column] == 0)
                {
                    for(int i = column + 1; i < this.grid[row].length; i++)
                    {
                        if (this.grid[row][i] != 0)
                        {
                            this.grid[row][column] = this.grid[row][i];
                            this.grid[row][i] = 0;
                            break;
                        }
                    }
