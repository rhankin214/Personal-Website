...
public class MyBSTIterator<K extends Comparable<K>, V> extends MyBST<K, V> {
    abstract class MyBSTNodeIterator<T> implements Iterator<T> {
         ...
         */
        public void remove() {
            //throws exception if last visited is null
            if (lastVisited == null) {
                throw new IllegalStateException();
            }
            //if last visited has two children (i.e. will get replaced
            //by another node in the tree before removal), sets the next pointer
            ...
            //sets the iterator's pointer to null. Importantly, does not set
            //the node itself to null, as that would cause problems with our
            //remove implementation since it doesn't fully remove and just
